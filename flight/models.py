...
class Flight(models.Model):
    ...
    def flight_info(self) -> str:
        return f"{self.route.source.location_city} -> {self.route.destination.location_city}"
    ...
    def __str__(self):
        return (
            f"Flight: {self.route.source.location_city} -> {self.route.destination.location_city}"
...
class Route(models.Model):
    ...
    def source_to_destination(self) -> str:
        return f"{self.source.location_city} -> {self.destination.location_city}"
...
class Airport(models.Model):
    ...
    closest_big_city = models.CharField(max_length=100, unique=False, null=True, blank=True)
...
class Ticket(models.Model):
    ...
    def validate_ticket(row, seat, airplane, type_error):
        ...
        for ticket_attr_value, ticket_attr_name, airplane_attr_name in [
            ...
            max_allowed_value = getattr(airplane, airplane_attr_name)
            if not (1 <= ticket_attr_value <= max_allowed_value):
                to_rise[airplane_attr_name] = f"{ticket_attr_name} must be between 1 and {max_allowed_value}"
    ...
    def __str__(self):
        return f"Ticket: {self.row} {self.seat} {self.flight.airplane} {self.order.id}"
